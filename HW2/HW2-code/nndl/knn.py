...
class KNN(object):
  ...
  def compute_L2_distances_vectorized(self, X):
    ...
    test_L2 = np.tile(np.sum(np.square(X), axis = 1), (num_train, 1))
    train_L2 = np.tile(np.sum(np.square(self.X_train), axis = 1), (num_test, 1))
    
    t = np.dot(X, self.X_train.T)
    
    dists = np.sqrt(test_L2.T + train_L2 - 2*t)
